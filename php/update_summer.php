            ...
            if ($scedual_row['count'] > 0) {
                ...
                if(str_contains($row['course_name'],'lab')||str_contains($row['course_name'],'Lab')||str_contains($row['course_name'],'LAB')){
                   foreach($halls['laboratory']as  $hall){
                    ...
                    $sql12 = "SELECT * FROM `tims`  where `course id`=".$lastsection['course_id']." and section=".(count($sections) + 1)." ";
                    $tims_serult = $conn->query($sql12);
                    $tims_row=[];
                    if($tims_serult->num_rows!=0){
                        $tims_row = $tims_serult->fetch_assoc();
                        $time=$tims_row['hour'];
                    }
                    ...
                    while ($row1=$result1->fetch_assoc())
                    {
                        ...
                        if($row1['time']==$time&&$row1['day']== $tims_row['day']&&$hall['hall_name']==$row1['room']){
                            
                        }else{
                            $update = "UPDATE `summer` SET time = ?,room=?,day=? WHERE id = ?";
                                $stmt = $conn->prepare($update);
                                $stmt->bind_param("sssi", $time,$hall['hall_name'],$tims_row['day'], $row['id']);
                                $stmt->execute();
                        }
                    }
                }
